...
void LoadTFList(char *filename, TF_list& List)
{
	...
	fread(List.arrNorm, sizeof(TF), List.size, fr);
	fread(List.arrTele, sizeof(TF), List.size, fr);
...
void SaveTFList(char *filename, TF_list List)
{
	...
	fwrite(List.arrNorm, sizeof(TF), List.size, fw);
	fwrite(List.arrTele, sizeof(TF), List.size, fw);
